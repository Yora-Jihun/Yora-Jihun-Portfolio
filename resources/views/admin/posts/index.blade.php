    ...
    <div class="min-h-screen">
        ...
        <main class="max-w-[1200px] mx-auto px-6 py-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
                ...
                <a href="{{ route('admin.posts.create') }}" class="bg-black text-white text-sm font-semibold py-2.5 px-5 rounded-lg hover:bg-gray-800 transition-all duration-200 shadow-sm hover:shadow-md w-full md:w-auto text-center">
                    New Post
                </a>
            </div>
            ...
            <div class="admin-table-wrapper admin-table-scroll border border-[#EAEAEA] rounded-xl overflow-hidden bg-white">
                <table class="w-full min-w-[640px]">
                    <thead>
                        <tr class="border-b border-[#EAEAEA]">
                            <th class="text-left px-4 py-3 md:px-6 md:py-4 text-[0.625rem] font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Title</th>
                            <th class="text-left px-4 py-3 md:px-6 md:py-4 text-[0.625rem] font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Category</th>
                            <th class="text-left px-4 py-3 md:px-6 md:py-4 text-[0.625rem] font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Status</th>
                            <th class="text-left px-4 py-3 md:px-6 md:py-4 text-[0.625rem] font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Published</th>
                            <th class="text-left px-4 py-3 md:px-6 md:py-4 text-[0.625rem] font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Featured</th>
                            <th class="text-left px-4 py-3 md:px-6 md:py-4 text-[0.625rem] font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Pinned</th>
                            <th class="text-right px-4 py-3 md:px-6 md:py-4 text-[0.625rem] font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-[#EAEAEA]">
                        @forelse($posts as $post)
                            <tr class="hover:bg-gray-50/80 transition-colors duration-150 group">
                                ...
                                <td class="px-4 py-3 md:px-6 md:py-4 text-sm text-gray-600">{{ $post->published_at ? format_post_date($post->published_at) : '—' }}</td>
                                <td class="px-4 py-3 md:px-6 md:py-4 text-sm text-gray-600">
                                    @if($post->featured)
                                ...
                                <td class="px-4 py-3 md:px-6 md:py-4 text-sm text-gray-600">
                                    @if($post->pin_post)
                                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-200">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                                ...
                                <td class="px-4 py-3 md:px-6 md:py-4 text-right text-sm">
                                    <div class="flex items-center justify-end gap-3">
                                        <a href="{{ route('admin.posts.edit', $post) }}" class="inline-flex items-center gap-1 text-[#16A34A] hover:text-emerald-700 font-medium transition-colors duration-200">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-8 md:px-6 md:py-12 text-center">
            ...
            <script>
                (function() {
                    var wrapper = document.querySelector('.admin-table-wrapper');
                    if (!wrapper) return;
